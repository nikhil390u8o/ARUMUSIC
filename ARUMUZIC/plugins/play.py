...
# --- THUMBNAIL FETCH (16:9) ---
def get_thumb(track: dict) -> str:
    """
    JioSaavn API se sabse badi image fetch karta hai.
    image array: [50x50, 150x150, 500x500]
    500x500 use karte hain - Telegram automatically 16:9 crop karta hai.
    """
    fallback = "https://files.catbox.moe/8wg1qy.jpg"
    try:
        images = track.get("image", [])
        if images:
            url = images[-1].get("url", fallback)
            # Force 500x500 - sabse badi size
...
async def play_next(chat_id: int):
    ...
    user_name = song["by"]
    thumbnail = song.get("thumb", "https://files.catbox.moe/8wg1qy.jpg")
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"https://jio-saa-van.vercel.app/result/?query={quote(query)}",
                timeout=aiohttp.ClientTimeout(total=15)
            ) as r:
                data = await r.json()
    except: 
        return await m.edit("❌ **Search Error!**")

    if not data: 
        return await m.edit("❌ **No results!**")
