...
def get_thumb(track: dict) -> str:
    ...
    try:
        # jio-saa-van API mein image field list hoti hai
        images = track.get("image", [])
        if isinstance(images, list) and images:
            url = images[-1].get("url", fallback) if isinstance(images[-1], dict) else str(images[-1])
...
async def play_cmd(client, msg: Message):
    ...
    try:
        ...
        stream_url = (
            track.get("media_url") or 
            track.get("download_url") or
            track.get("perma_url") or
            ""
        )

        if not stream_url:
            return await m.edit("❌ **Stream URL nahi mila!**")
