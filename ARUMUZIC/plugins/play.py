...
async def update_timer(chat_id, message_id, duration):
    ...
    while True:
        await asyncio.sleep(12) # Update interval
        if chat_id not in config.queues or not config.queues[chat_id]: break
        
        elapsed_time = int(time.time() - start_time)
        
        # Agar song khatam hone wala hai, toh full bar dikha kar stop karo
...
async def vplay_cmd(client, msg: Message):
    ...
    m = await msg.reply("<blockquote>🎬 <b>ғᴇᴛᴄʜɪɴɢ sᴛʀᴇᴀᴍ...</b></blockquote>")
    ...
    try:
        ...
        thumb_url = data.get("thumbnail") # YouTube Rectangle
    ...
    if len(config.queues[chat_id]) > 0:
        ...
        return await m.edit(f"<b>✅ ᴀᴅᴅᴇᴅ ᴛᴏ ǫᴜᴇᴜᴇ</b>\n🎵 {title}")
